  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-[28px] border border-line bg-white shadow-soft">
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        ...
        <div>
          <h3 className="min-h-[4.75rem] font-display text-3xl leading-tight text-ink">{project.name}</h3>
          <p className="mt-2 min-h-[3rem] text-sm leading-7 text-steel">{project.address}</p>
        </div>
        <HtmlContent
          html={project.description}
          className="prose prose-sm min-h-[6.75rem] max-w-none overflow-hidden text-steel prose-p:my-0 prose-p:leading-7 prose-li:leading-7 prose-strong:text-ink [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:3]"
        />
        <Link
