...
function formatField(value: string | null | undefined, fallback = "Chưa cung cấp") {
...
function createEmailSubject(payload: ContactNotificationPayload) {
  return `[WhiteSpace] Liên hệ mới từ ${payload.name}`;
}

function createEmailText(payload: ContactNotificationPayload) {
  return [
...
function createEmailHtml(payload: ContactNotificationPayload) {
